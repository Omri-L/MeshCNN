...
class MeshPool(nn.Module):
    ...
    def __pool_edge(self, mesh, edge_id, mask, edge_groups):
        a = self.__is_valid_config(mesh, edge_id)
        if self.has_boundaries(mesh, edge_id):
            ...
            self.__merge_edges = self.__pool_side(mesh, edge_id, mask,
                                                  edge_groups, 0)
    ...
    @staticmethod
    def rotate_edges_and_connections(mesh, fe, origin_vertex, edges_to_change,
                                     optional_vertices,
        ...
        for face_id, face in enumerate(faces_vertices):
            ...
            for idx, edge in enumerate(faces_edges):
                ...
                edge_nb[edge_key][nb_count[edge_key] + 1] = edge2key[
                    faces_edges[(idx + 2) % 4]]
                edge_nb[edge_key][nb_count[edge_key] + 2] = edge2key[
                    faces_edges[(idx + 3) % 4]]
                nb_count[edge_key] += 3
            for idx, edge in enumerate(faces_edges):
                edge_key = edge2key[edge]
    ...
    def clear_doublets(self, mesh):
        ...
        self.clear_doublets(mesh)


    def rotate_edges(self, mesh, fe, edge_id):
        ...
        # for each edge goes from u - change the original connection to the
        # optional diagonal connection
        mesh, new_features_combination_dict, new_features_dict = \
            self.rotate_edges_and_connections(mesh, fe, u, e_u, diag_vertices,
                                              diag_vertex_to_edges_dict)

        # collapse another 2 edges connected to v
